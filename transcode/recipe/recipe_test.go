...
func TestRecipeParserRejectsUnsafeOrUnknownContent(t *testing.T) {
	...
	cases := []struct{ name, data, want string }{
		...
		{"unsafe bundle version", strings.Replace(base, `bundle_version: "2026.09.1"`, `bundle_version: ".."`, 1), "invalid bundle_version"},
...
func versionedBundle(version string, quality int) []byte {
	...
	s = strings.Replace(s, `bundle_version: "2026.09.1"`, fmt.Sprintf(`bundle_version: %q`, version), 1)
...
func TestManagerDoesNotRestoreCacheFromDifferentSource(t *testing.T) {
	...
	if m2.Snapshot().Identity.Version != "2026.09.1" {
